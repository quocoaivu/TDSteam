...
namespace Data
{
	public class FreeResourcesStore : MonoBehaviour
	{
        ...
        private FreeResourcesRecord data = new FreeResourcesRecord();

        private static FreeResourcesStore instance;
		...
		public static FreeResourcesStore Instance
		{
			get
			{
				return FreeResourcesStore.instance;
			}
		}

		private void Awake()
		{
			FreeResourcesStore.instance = this;
			SaveDefaultData();
			Load();
		}
		...
		{
			if (!File.Exists(Application.persistentDataPath + FreeResourcesStore.DB_NAME))
			{
				FileStream fileStream = File.Create(Application.persistentDataPath + FreeResourcesStore.DB_NAME);
		...
		private void Load()
		{
			string path = Application.persistentDataPath + FreeResourcesStore.DB_NAME;
			try
			{
				using (FileStream fileStream = File.Open(path, FileMode.Open))
				{
					BinaryFormatter binaryFormatter = new BinaryFormatter { Binder = new Common.SaveTypeCompatBinder() };
					data = (FreeResourcesRecord)binaryFormatter.Deserialize(fileStream);
				}
			}
			catch (Exception e)
			{
				UnityEngine.Debug.LogError("FreeResourcesStore: failed to load save, regenerating default. " + e.Message);
				if (File.Exists(path))
				{
					File.Delete(path);
				}
		...
		public void RestoreDataFromCloud(PlayerRecord_FreeResources restoredData)
		{
