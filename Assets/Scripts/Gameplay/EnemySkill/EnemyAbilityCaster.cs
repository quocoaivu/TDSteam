...
namespace Gameplay
{
	public class EnemyAbilityCaster : EnemyBrain
	{
        [SerializeField]
        private EnemyAbilityBase enemySkill;

        [Space]
        [SerializeField]
        private OrderedUnityEvent onSkillActivated = new OrderedUnityEvent();

        [SerializeField]
        private OrderedUnityEvent onSkillDeactivated = new OrderedUnityEvent();
		...
		{
			StopSkill();
			coolDownTimeTracking = (float)(-(float)cooldownIncreaseTime);
