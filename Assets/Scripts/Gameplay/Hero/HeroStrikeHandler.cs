...
namespace Gameplay
{
	public class HeroStrikeHandler : HeroHandler
	{
        [Header("Specific")]
        [SerializeField]
        private List<ProjectileSpec> bulletParametersInOneTurn = new List<ProjectileSpec>();
		...
		public float AttackRangeMax
		{
			get
			{
				return (float)base.HeroModel.OriginalParameter.attack_range_max / GameRecord.PIXEL_PER_UNIT;
			}
		}
		...
		public float AttackRangeAverage
		{
			get
			{
				return (float)base.HeroModel.OriginalParameter.attack_range_average / GameRecord.PIXEL_PER_UNIT;
			}
		}
		...
		public float AttackRangeMin
		{
			get
			{
				return (float)base.HeroModel.OriginalParameter.attack_range_min / GameRecord.PIXEL_PER_UNIT;
			}
		}
		...
		public override void OnAppear()
		{
			...
			{
				MonoSingleton<BulletPool>.Instance.InitBulletsFromHeroes(base.HeroModel.OriginalParameter.id, 0);
			}
		...
		{
			CooldownTime = (float)base.HeroModel.OriginalParameter.attack_cooldown / 1000f;
		...
		{
			if (base.HeroModel.currentTarget)
			{
		...
		private void ApplyIncreaseAttackSpeed()
		{
			...
			cooldownTime = (float)base.HeroModel.OriginalParameter.attack_cooldown / 1000f - (float)base.HeroModel.OriginalParameter.attack_cooldown * buffsValue / 100f / 1000f;
			cooldownTime = Mathf.Clamp(cooldownTime, 0.1f, 999f);
		}

		private void ApplyIncreasePhysicsDamage()
		{
			if (HeroParameterManager.Instance.IsPhysicsAttack(base.HeroModel.HeroID))
			{
		...
		{
			if (HeroParameterManager.Instance.IsMagicAttack(base.HeroModel.HeroID))
			{
		...
		public void DamageToEnemy(EnemyData enemy)
		{
			// Guard against a prefab left with attackFrame=0 (integer divide-by-zero).
			// Reachable even for ranged heroes pulled into melee (RangeAtk->MoveToTarget->MeleeAtk).
			int frames = Mathf.Max(1, attackFrame);
			enemy.ProcessDamage(DamageKind.Melee, new SharedStrikeDamage
			{
				physicsDamage = UnityEngine.Random.Range(atkPhysicsMin / frames, atkPhysicsMax / frames),
				magicDamage = UnityEngine.Random.Range(atkMagicMin / frames, atkMagicMax / frames),
				criticalStrikeChance = CurrentCriticalStrikeChance,
				damageSource = CharacterKind.Hero,
				sourceId = base.HeroModel.HeroID
			});
		}
		...
		private IEnumerator CreateBullet(ProjectileSpec bulletParameter)
		{
			...
			if (base.HeroModel.currentTarget)
			{
				HeroSpec originalParameter = base.HeroModel.OriginalParameter;
				ProjectileEntity forHero = MonoSingleton<BulletPool>.Instance.GetForHero(originalParameter.id, 0);
