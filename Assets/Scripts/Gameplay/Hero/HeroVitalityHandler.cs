...
namespace Gameplay
{
	public class HeroVitalityHandler : HeroHandler
	{
        ...
        private TrooperVitalityView heroHealthView;
		...
		{
			base.OnAppear();
			OriginHealth = base.HeroModel.OriginalParameter.health;
			CurrentHealth = OriginHealth;
			OriginPhysicsArmor = (float)base.HeroModel.OriginalParameter.armor_physics / 100f;
			OriginMagicArmor = (float)base.HeroModel.OriginalParameter.armor_magic / 100f;
			onAppearEvent.Dispatch();
			CurrentPhysicsArmor = OriginPhysicsArmor;
			CurrentMagicArmor = OriginMagicArmor;
			healthRegenValue = base.HeroModel.OriginalParameter.health_regen;
			timeTracking = (float)base.HeroModel.OriginalParameter.health_regen_cooldown / 1000f;
		...
		{
			heroHealthView = MonoSingleton<UnitHealthBarPool>.Instance.Get();
			heroHealthView.SetupHealth(CharacterKind.Ally, base.gameObject, healthBarPosition);
		}
		...
		public void UpdateHealthView()
		{
			...
			MonoSingleton<GameplayUIHeroDirector>.Instance.UpdateHeroHealthBarStatus(base.HeroModel.OriginalParameter.id, CurrentHealth, OriginHealth);
		}
		...
		[SerializeField]
		private OrderedUnityEvent onAppearEvent;
