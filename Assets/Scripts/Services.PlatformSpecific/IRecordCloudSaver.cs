...
{
	public interface IRecordCloudSaver
	{
		...
		void RetrieveData(string dbRef, Action<IRecordSnapshot> callback);

		void RetrieveDataWithMainThreadCallback(string dbRef, Action<IRecordSnapshot> callback);

		void UpdateData(Dictionary<string, object> updateList, string dbRefPath = null);

		void WriteData(object data, string dbRefPath);

		void WriteDataWithMainThreadCallback(object data, string dbRefPath, Action<IRecordSnapshot> callback);
